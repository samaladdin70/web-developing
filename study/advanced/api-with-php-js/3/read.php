...
if ($database) {
    ...
    $readData->execute();
    $readData = $readData->fetchAll(PDO::FETCH_ASSOC);
    ...
    foreach (array_reverse($readData) as $value) { // to reverse excution from end to begin use array_reverse()
        // print_r($value);
        echo ("<div style='width:96%; height:250px; overflow:auto; text-align:right;' 
        class='card-body card-header card text-white bg-success mb-3'>
        <h1 style='color:black; font-wight:bold; text-shadow:2px 2px 4px black;' class='card-title'>" . $value["title"] . "</h1>" .
