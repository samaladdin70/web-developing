...
if (isset($_POST['insertLib'])) {
    ...
    $connect = new  PDO("mysql:host=sql205.eb2a.com;", $username, $password);
    if ($connect) {
        echo '<h2 style="color:seagreen;">تم الإتصال بنجاح بخادم قواعد البيانات</h2>';
        $connect = $connect->prepare("CREATE DATABASE  IF NOT EXISTS eb2a_29357997_liberary_db CHARACTER SET utf8 COLLATE utf8_general_ci;");
        $connect->execute();
